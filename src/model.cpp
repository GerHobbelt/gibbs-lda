...
std::vector<double> model::get_topicdistribution(std::string clickstream) {
    ...
    srandom(12345); // initialize for random number generation
    newz = new int *[newM];
    ...
    for (inf_liter = 1; inf_liter <= niters; inf_liter++) {
        ...
        for (int m = 0; m < newM; m++) {
            for (int n = 0; n < pnewdata->docs[m]->length; n++) {
                // loop though all new words of the new document
                // (newz_i = newz[m][n])
                // sample from p(z_i|z_-i, w)
    ...
    std::vector<double> theta = get_newtheta();
    return theta;
}
...
std::vector<double> model::get_newtheta() {
    ...
    for (int m = 0; m < newM; m++) {
        for (int k = 0; k < K; k++) {
            newtheta.push_back((newnd[m][k] + alpha) / (newndsum[m] + K * alpha));
        }
    }
    return newtheta;
}

std::vector<std::vector<double>> model::get_phi() {
    std::vector<std::vector<double>> lambda;
    std::vector<std::vector<double>> temp(V,std::vector<double>(K));
    lambda.swap(temp);
    for (int k = 0; k < K; k++) {
        for (int w = 0; w < V; w++) {
            lambda[w][k] = (nw[w][k] + beta) / (nwsum[k] + V * beta);
        }
    }
    return lambda;
}
